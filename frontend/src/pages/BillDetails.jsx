...
export function BillDetails({ id }) {
    ...
    if (!bill) {
        return (
            <div>
                ...
                <div class="text-center text-danger-dark">The requested bill could not be found.</div>
            </div>
        )
    }
    ...
    return (
        <div>
            ...
            {error && <div class="text-center text-danger-dark">{error}</div>}

            {/* Bill Information Card */}
            <div
                class={`relative mb-8 cardstatic p-8 transition-all ${
            ...
            >
                <div class="mb-8 flex items-center justify-between border-b border-gray-200 pb-4">
                    <h3 class="m-0 text-3xl font-bold text-primary">{bill.name}</h3>
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">Amount</label>
                        <div class="text-3xl font-bold text-primary">${bill.amount.toFixed(2)}</div>
                    </div>

                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">Recurrence Type</label>
                        <div class="text-lg font-medium text-gray">
                    ...
                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">
                    ...
                    {(bill.recurrence_type === 'interval' || bill.recurrence_type === 'none') && bill.start_date && (
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="text-lg font-medium text-gray">{formatDate(bill.start_date)}</div>
                        </div>
                    )}

                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">Next Due Date</label>
                        <div
                            class={`text-lg font-medium ${
                                getBillStatus(bill) === 'overdue'
                                    ? 'font-semibold text-danger-dark'
                                    : getBillStatus(bill) === 'due-today'
                    ...
                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">Last Paid</label>
                        <div class="text-lg font-medium text-gray">
                    ...
                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">Category</label>
                        <div class="text-lg font-medium text-gray">{bill.category || 'None'}</div>
                    </div>

                    {bill.notes && (
                        <div class="col-span-full flex flex-col gap-2">
                            <label class="text-xs font-semibold tracking-wide text-gray uppercase">Notes</label>
                            <div class="text-base leading-relaxed text-gray italic">{bill.notes}</div>
                        </div>
                    )}

                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">Created</label>
                        <div class="text-lg font-medium text-gray">{formatDateTime(bill.created_at)}</div>
                    </div>

                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-semibold tracking-wide text-gray uppercase">Last Updated</label>
                        <div class="text-lg font-medium text-gray">{formatDateTime(bill.updated_at)}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="cardstatic p-8">
                <div class="mb-8 flex items-center justify-between border-b border-gray-200 pb-4">
                    <h3 class="m-0 text-2xl font-bold text-primary">Payment History</h3>
            ...
            {showDeleteConfirm && paymentToDelete && (
                ...
                >
                    ...
                    <div class="mb-4 rounded-md bg-card-bg p-4 font-mono text-sm">
                        <strong>Amount:</strong> ${paymentToDelete.amount.toFixed(2)}
                        <br />
                        <strong>Date:</strong> {formatDate(paymentToDelete.payment_date)}
                    </div>
                    <p class="font-medium text-danger">This action cannot be undone.</p>
                </ConfirmationModal>
            )}
        </div>
    )
