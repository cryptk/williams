...
export function Bills() {
  ...
  const loadBills = async () => {
    try {
      ...
      console.error('Failed to load bills:', error) // eslint-disable-line no-console -- We only allow console logging for debugging purposes
    } finally {
      setLoading(false)
    }
  }
  ...
  const loadCategories = async () => {
    try {
      ...
      console.error('Failed to load categories:', error) // eslint-disable-line no-console -- We only allow console logging for debugging purposes
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const billData = {
        ...
        recurrence_days: parseInt(formData.recurrence_days, 10),
      ...
      toast.error(error.message || `Failed to ${editingBill ? 'update' : 'create'} bill`)
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleMarkAsPaid = async (bill) => {
    ...
    try {
      ...
      console.error('Failed to record payment:', error) // eslint-disable-line no-console -- We only allow console logging for debugging purposes
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      console.error('Failed to delete bill:', error) // eslint-disable-line no-console -- We only allow console logging for debugging purposes
