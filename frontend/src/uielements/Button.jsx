export default function Button({ children, variant = 'primary', extraClasses = '', ...props }) {
  const variantMap = {
    ...
    secondaryHollow: 'bg-transparent border-2 border-secondary text-secondary-dark hover:bg-secondary hover:text-white',
  ...
  return (
    <>
      <button
        type="button"
        class={`${variantClasses} drop-shadow-secondary hover:drop-shadow-secondary inline-block rounded-md px-4 py-2 text-base font-semibold drop-shadow-md transition-all hover:scale-103 hover:drop-shadow-lg ${extraClasses}`}
        {...props}
      >
        {children}
      </button>
